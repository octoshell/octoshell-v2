module Pack
  ...
  describe "Pack::Access without date" do
    {"requested" => [ 'denied' ],"allowed" => ["denied"],"denied" => [] }.each do |key,value|
      describe "acesses_actions_for_#{key}_status" do
        before(:each) { @access = access_with_status(status: key, end_lic: AmericanDate.yesterday.to_s) }
        describe "date" do
          it "shows actions" do
