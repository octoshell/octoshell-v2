 create_table "pack_accesses", force: :cascade do |t|
    ...
    t.date     "new_end_lic" # по какое число пользователь хочет продлить доступ 
  end
 ...
 # Модель ниже предназначена для запоминания предпочтений пользователя при запросах на доступ. Возможно стоит удалить и перенести это в сессии
  ...
  create_table "pack_versions", force: :cascade do |t|
    ...
    t.string   "folder"
    t.date     "end_lic"
    t.string   "state" # наличие лицензии 
    t.integer  "lock_version", default: 0, null: false # планирую применить оптимистичную блокировку
    t.boolean  "deleted"
  end
