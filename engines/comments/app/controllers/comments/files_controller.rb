  ...
  class FilesController < ApplicationController
    def index
      if params[:attach_to][:class_name] == 'all'
        @q = FileAttachment
        if params[:q]
          @q = @q.where(context_id: q_params[:context_id_eq]) if q_params[:context_id_eq] != 'all'
          @q = @q.where(attachable_type: q_params[:attachable_type_eq]) if q_params[:attachable_type_eq] != 'all'
          @q = @q.where(attachable_id: q_params[:attachable_id_eq]) unless q_params[:attachable_id_eq].blank?
        end
