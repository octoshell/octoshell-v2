if Core.user_class
  Core.user_class.class_eval do
    ...
    has_many :invitational_account, -> { where(project_access_state: 'invited') },
             class_name: '::Core::Member',
             foreign_key: :user_id
    has_many :projects_with_invitation, -> { where.not(state: %w[cancelled closed]) },
    ...
    scope :with_active_projects, lambda {
      joins(accounts: :project).where(core_projects: { state: 'active' }, core_members: { project_access_state: 'allowed' }).distinct
    }
    ...
    # Prevent multiple inclusion of AASM in development mode
    unless self < AASM
      include AASM
