    ...
    def index
      respond_to do |format|
        format.html do
          ...
          @count_all_members = Member.select('core_members.project_id, COUNT(id) as count_p')
                                     .group(:project_id)
          @count_allowed_members = Member.select('core_members.project_id, COUNT(id) as count_p')
                                         .group(:project_id)
          if display_all_applied?
            @count_all_members = @count_all_members.where(project_id: @projects.map(&:id))
            @count_allowed_members = @count_allowed_members.where(project_id: @projects.map(&:id))
          else
