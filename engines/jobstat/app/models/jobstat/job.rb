...
module Jobstat
  class Job < ApplicationRecord
    ...
    def get_performance
      data = FloatDatum.where(job_id: id).map { |h| [h.name.to_sym, h.value] }.to_h

      %i[ib_rcv_data_fs ib_xmit_data_fs ib_rcv_data_mpi
         ib_xmit_data_mpi lustre_read_bytes lustre_write_bytes].each do |a|
        data[a] /= 1024 * 1024 unless data[a].nil?
      end
    ...
    def get_ranking
...

      {
        ...
        instructions: get_one_rank(performance[:instructions], low_str, 100_000_000, average_str, 400_000_000,
                                   high_str),
        gpu_load: get_one_rank(performance[:gpu_load], low_str, 20, average_str, 80, high_str),
        loadavg: (if cluster == 'lomonosov-1'
        ...
        ib_xmit_data_fs: get_one_rank(performance[:lustre_write_bytes] || performance[:ib_xmit_data_fs], low_str, 10, average_str, 100,
                                      high_str),
        ib_rcv_data_fs: get_one_rank(performance[:lustre_read_bytes] || performance[:ib_rcv_data_fs], low_str, 10,
                                     average_str, 100, high_str),
    ...
    def priority_filtration(entries)
      ...
      entries.each do |condition|
        ...
        max_priority[condition['group']] = condition['priority'] if condition['priority'] > current_priority
      end

      entries.each do |condition|
        result.push(condition) if condition['priority'] >= max_priority[condition['group']]
      end
    ...
    def get_not_public_by_type(types)
      ...
      types.each do |section_name|
        (Job.rules[section_name] || {}).each do |condition, fields|
          result << condition if fields['public'] == 0
        end
      end
      result
    end
    ...
    def get_rules(user)
      ...
      filters = filters[-1]['filters'] || [] if filters.length > 0
      ...
      result.each do |type|
        type['modules'] = type['module_recommendation'].split(',').map do |x|
          Job.rules['detailed_analysis_types'][x] || {}
    ...
    def get_extra_data
      ...
      return JSON.parse(extra_data.value) if extra_data
    ...
    def self.get_feedback_job(user_id, joblist = [])
      jobs = joblist.is_a?(Array) ? joblist.join(',') : joblist
    ...
    rescue StandardError => e # Net::ReadTimeout, Net::OpenTimeout
    ...
    def get_command
      array = command.split
      if %r{/ompi$}.match(array[0]) ||
         %r{/impi$}.match(array[0]) ||
         %r{/run$}.match(array[0])
        array.shift
      end
      if %r{([^/]+)$}.match array[0]
    ...
    def self.rules(force_reload = false)
      if @rules.nil? || force_reload
        @rules = {}
        begin
          File.open('engines/jobstat/config/conditions.json', 'r') do |file|
