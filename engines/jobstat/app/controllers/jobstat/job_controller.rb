module Jobstat
  class JobController < ApplicationController
    ...
    def show
      ...
      @jobs_plus = { @job.drms_job_id => %i[id cluster drms_job_id drms_task_id login partition submit_time start_time
                                            end_time timelimit command state num_cores num_nodes].map do |i|
        [i.to_s, @job[i]]
      end.to_h }
      ...
      @loadavg_digest_data = graph_data_single(DigestFloatDatum.where(job_id: @job.id,
                                                                      name: 'loadavg').order(:time).all)
      ...
      rcv_fs = DigestFloatDatum.where(job_id: @job.id, name: 'lustre_read_bytes').order(:time).all
      xmit_fs = DigestFloatDatum.where(job_id: @job.id, name: 'lustre_write_bytes').order(:time).all

      rcv_fs = DigestFloatDatum.where(job_id: @job.id, name: 'ib_rcv_data_fs').order(:time).all unless rcv_fs.any?
      xmit_fs = DigestFloatDatum.where(job_id: @job.id, name: 'ib_xmit_data_fs').order(:time).all unless xmit_fs.any?
      ...
      return unless @job_extra_data.present?

      @job_rules_description.each do |rule_descr|
        next unless @job_extra_data.key?(rule_descr['name'])
    ...
    def detailed
      ...
      @detailed_types_count = (Job.rules['detailed'][params[:analysis_id]] || {}).reject do |_, obj|
        obj['public'] == 0
      end.length || 0
      @analysis_id = params[:analysis_id]
      @job = Job.find(params[:id])
      return unless @detailed_info.nil?
